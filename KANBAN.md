...
- **Named versions — wire + SDK + durability** — the `Hub` index landed (#86, in-memory). Next buildable slices: wire messages (create/list/rename/delete + fetch a version's state) so a client drives it, and the SDK surface over them; then durable persistence of the index in the `Store` (versions are "retained until the app deletes", but the store only holds op log + compaction snapshot today). Restore-as-branch and auto-version triggers stay blocked (branch layer / engine-event hooks). → *Versioning*. (v0.2)
