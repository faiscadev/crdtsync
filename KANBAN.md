...
- **Schema layer — the opt-in feature tier that unblocks awareness-TTL, `@auth`, invariant repair, and migration** (§Schema, §Schema Migration; design settled 2026-07-03, DECISIONS). The distribution/trust/versioning model is now pinned (schema is code, registered per `app_id`, relay-vs-enforcing tiers, client-declared version ranges + per-edge compat class + per-recipient in-flight translation, `onRepaired`/`onOpsRejected`/`onUpdateRequired` hooks). Sliced dependency-order; the first units consume only **built** primitives (map/list/text/register/counter + awareness), so they land now — marks/attrs/structural clauses parse but stay enforcement-inert until XmlElement/marks exist:
  - **Unit 1 — schema parse + validation engine (core)**: `core::schema` parses the JSON file → a `Schema`, validates it at parse time (closure-of-violations: every declared dimension maps to a repair rule, so no schema admits an unrepairable state), and validates an op / materialized state against it. Built-primitive constraints only (`root`/`types` for map/list/text/register/counter, `min`/`max`, `children` cardinality). The sole validator — SDKs forward bytes, never reimplement. Foundation for every unit below.
  - **Unit 2 — invariant repair (built-primitive rules)**: deterministic convergent repair at the apply boundary — out-of-range scalar/counter clamp, exclusive-collision keep-lamport-oldest, disallowed-value drop — plus the `onRepaired` observation event. Convergence-harness tested (all replicas repair identically given a shared schema).
  ...
  - **Unit 4 — schema-level `@auth`**: parse `auth.roles`/`auth.grants` (with `${actor_id}`/`${author_id}` templating) into the schema-default tier feeding the #84 ACL evaluator (explicit-deny → allow → schema-grant → default-deny). **Unblocks the schema half of the queued authz layer.**
  - **Unit 5 — schema registration + trust (server)**: admin API ingesting `{app_id, version, schema, migrations}` into a per-`app_id` registry; meta-auth gate on registration; hash-lock; handshake resolves a client's asserted `app_id`+version to the registered schema (unknown → reject). Establishes the enforcing tier + trust boundary.
  - **Unit 6 — handshake schema advertisement + version negotiation**: enforcing server advertises its active schema+version; client caches across restart; client declares on handshake the single version it wants ops delivered at (server is the compatibility layer — a "range of one" normally); relay stays schema-free.
  - **Unit 7 — migration engine (core)**: versioned transforms with the built-in step kinds, each carrying a bidirectional op-rewrite (forward always; inverse iff back-compat); back-compat vs breaking edge classification; migration-as-log-entries; the four detection gates (CI drift/verify, boot hash-lock, per-doc reachability).
