...
**Named versions (server handling)** — `step` serves the five requests on the channel's room: mutations run the `Hub` op and reply the fresh `Versions` list; list replies the same; fetch replies `VersionState` (hit) or the list (miss). Gated on `Write` (mutations) / `Read` (list/fetch); denial → non-closing `Forbidden`, unbound channel → violation, persist failure → closing `Internal` (#89). Client `ClientSession` view + SDK surface are the closing slices (see Next). Restore-as-branch + auto-triggers stay blocked.
...
- **Named versions — client + SDK** — the `Hub` index (#86), durable persistence (#87), wire (#88), and server handling (#89) landed. Next buildable slices: the client `ClientSession` version issue-methods (create/rename/delete/list/fetch → the request frames) + folding `Versions`/`VersionState` replies into a per-channel version view exposed via getters, then the FFI/Python/Go/wasm surface over it. Restore-as-branch and auto-version triggers stay blocked (branch layer / engine-event hooks). → *Versioning*. (v0.2)
