...
- **Op acknowledgement — the gate that unlocks offline-queue + tombstone-GC** (§Op Acknowledgement, opened 2026-07-02, DECISIONS). One ack concept, two directions, sliced dependency-order:
  ...
  - **Unit 2 — client outbox / offline queue — DONE (#PR)**: `ClientSession` retains authored ops per channel; `edit`/`atomic_edit`/`commit_atomic` enqueue; `receive(Accepted{through})` prunes `id.seq <= through`; `resend(channel)` re-emits the unpruned tail; `outbox_len(channel)` exposes the queue depth. `client_outbox.rs`. Delivers the **offline-queue** half at the client seat (the server ack that drives it is Unit 3).
...
- **Tombstone GC / watermark** — now **Unit 4 of the Op Acknowledgement gate** (see Next). The ack-protocol blocker is resolved: the watermark input is the client `Ack` (server seq) + the per-client last-acked registry, and the retention-horizon eviction policy is decided (DECISIONS, 2026-07-02). The remaining depth is the **leaf-only, anchor-aware** tombstone drop (drop a below-watermark tombstone only when no surviving node parents off it — not a flat "discard older than watermark"), gated on the convergence harness (invariant: GC preserves materialized state). Time/migration compaction triggers + retention window ("keep last N") ride along. → *Snapshots / Tombstone GC*. (v0.2, unblocked — builds after Units 1–3)
