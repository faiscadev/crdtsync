...
- **Atomic transactions — client-seat SDK surface** — core (#102), drive path (#103), and the doc-level begin/commit SDK surface (#104) landed. Remaining: per-channel atomic wrappers over `ClientSession::atomic_edit` in the SDK `Client` types (begin/commit on a channel returning the Ops frame). Atomic-tx **undo** ("a transaction is naturally an undo intention") and scope constraints (one branch/zone/schema version, member cap) are gated on the branch/zone/schema layers. → *Transactions*. (v0.2)
