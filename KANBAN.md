...
- **Schema-aware diff — language SDKs + blocked detail** — the core diff spans every built primitive (#109/#110/#111), serializes to bytes (#112), and crosses the C ABI via `crdtsync_diff` (#113). Remaining: the **Python/Go decoders** (over `crdtsync_diff` — decode the change-list buffer into native structures) + **wasm** (over `core::diff` directly), each pairing with `version_state` + export; default **renderers** (ARCHITECTURE §Schema-Aware-Diff "engine ships sensible default renderers; apps can override"); and **XmlElement / marks / attrs diffs** (blocked — those primitives aren't built). → *Versioning / Schema-Aware Diff*. (v0.2, partly blocked)
