...
**Schema-aware diff (core)** — `core::diff::diff(old, new) -> Vec<Change>` computes the structural changes between two replica snapshots by walking the Element trees in lockstep (#109): slots `Added`/`Removed` (with kind), scalar/register `Value` and `Counter` old→new value changes, nested maps recursed so a deep edit reports at its own path, and List/Text as a coarse `Sequence` marker (element-/char-level detail deferred). Path-addressed (shared `path` encoding) and ordered by path, so diffing a pair is deterministic. Backed by `Map::keys()`. Pairs with named versions + export/import (compare any two snapshots). Element-/char-level sequence diffs, XmlElement/marks/attrs diffs (unbuilt primitives), and the SDK surface are follow-ons (see Queue).
