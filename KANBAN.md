...
**Tombstone compression** — sequence tombstones (List/Text) are compressed, never removed (#PR). The state codec encodes live nodes in full, then run-length-encodes contiguous deleted runs (consecutive stamps on one insert's parent-chain) into single `(start, length, anchor)` range records and drops dead values; a deleted region costs O(runs) not O(deleted items). `STATE_VERSION` → 2; Text decode validates live codepoints only; a run past a cap splits on encode and is rejected on decode (decompression-bomb guard). Convergence-trivial (identical logical state, fewer bytes). Replaces the abandoned watermark-removal GC (see DECISIONS 2026-07-02); the `Ack` wire frame is now a reserved no-consumer slot.
