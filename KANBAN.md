...
**Named versions (FFI)** — the `CrdtClient` C ABI exposes the five issue methods (`crdtsync_client_{create,rename,delete,list,fetch}_version` → request frames) and the view (`version_count` + indexed `version_name`; `version_state` by name) (#91). Header symbols asserted; Miri leak-clean. Python/Go/wasm layer over it next (see Next). Restore-as-branch + auto-triggers stay blocked.
...
- **Named versions — language SDKs** — the arc through the FFI surface landed (#86–#91). Closing slice: wrap the version issue-methods + view (`create/rename/delete/list/fetch` + `version_count`/`version_name`/`version_state`) in the Python, Go, and wasm SDKs over the C ABI (wasm over `ClientSession` directly). Restore-as-branch and auto-version triggers stay blocked (branch layer / engine-event hooks). → *Versioning / SDKs*. (v0.2)
