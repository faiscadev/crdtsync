...
- **Atomic transactions — DONE (#102–#105)** — core (`Tx{id,count}` envelope, buffer-until-whole, seq-order commit), drive path (`ClientSession::atomic_edit`; server unchanged), and the SDK surface at both the doc seat (`begin_atomic`/`commit_atomic` over FFI + Python/Go/wasm) and the client seat (per-channel begin/commit). All-or-nothing view boundary, convergence preserved, verified end to end. Remaining refinements are gated: atomic-tx **undo** ("a transaction is naturally an undo intention") needs undo↔tx integration, and scope constraints (one branch/zone/schema version, member cap) need the branch/zone/schema layers. → *Transactions*. (v0.2)
