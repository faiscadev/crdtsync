...
- **Schema-aware diff — language SDKs + blocked detail** — the core diff spans every built primitive (#109/#110/#111), serializes to bytes (#112), and crosses the C ABI via `crdtsync_diff` (#113). Remaining: the **Go decoder** (over `crdtsync_diff` — decode the change-list buffer, mirroring the wasm/Python `{op,path,...}` + tagged-scalar shape from #115/#116); default **renderers** (ARCHITECTURE §Schema-Aware-Diff "engine ships sensible default renderers; apps can override"); and **XmlElement / marks / attrs diffs** (blocked — those primitives aren't built). → *Versioning / Schema-Aware Diff*. (v0.2, partly blocked)
