...
- **Schema-aware diff — SDK surface + blocked detail** — the core diff spans every built primitive (#109/#110/#111). Remaining: the **SDK surface** (expose `diff` over FFI/wasm/Python/Go against two state blobs — pairs with `version_state` + export; needs a change-list marshalling shape across the C ABI), default **renderers** (ARCHITECTURE §Schema-Aware-Diff "engine ships sensible default renderers; apps can override"), and **XmlElement / marks / attrs diffs** (blocked — those primitives aren't built). → *Versioning / Schema-Aware Diff*. (v0.2, partly blocked)
