...
- **Schema-aware diff — SDK surface + blocked detail** — the core diff spans every built primitive (#109/#110/#111) and serializes to bytes (#112). Remaining: the **FFI entry point** (`crdtsync_diff(old_state, new_state) -> CrdtBuf` returning the encoded change list) + **Python/Go/wasm decoders** (decode the change-list buffer into native structures — pairs with `version_state` + export), default **renderers** (ARCHITECTURE §Schema-Aware-Diff "engine ships sensible default renderers; apps can override"), and **XmlElement / marks / attrs diffs** (blocked — those primitives aren't built). → *Versioning / Schema-Aware Diff*. (v0.2, partly blocked)
