...
- **Named versions — SDK surface** — the `Hub` index (#86), durable persistence (#87), wire (#88), server handling (#89), and client `ClientSession` view (#90) landed. Closing slice: expose the version issue-methods + view over the `CrdtClient` C ABI (FFI), then Python / Go / wasm. Restore-as-branch and auto-version triggers stay blocked (branch layer / engine-event hooks). → *Versioning / SDKs*. (v0.2)
