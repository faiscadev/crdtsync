...
**Schema-aware diff (core)** — `core::diff::diff(old, new) -> Vec<Change>` computes the structural changes between two replica snapshots by walking the Element trees in lockstep (#109): slots `Added`/`Removed` (with kind), scalar/register `Value` and `Counter` old→new value changes, nested maps recursed so a deep edit reports at its own path, and List as a coarse `Sequence` marker. Path-addressed (shared `path` encoding) and ordered by path, so diffing a pair is deterministic. Backed by `Map::keys()`. Pairs with named versions + export/import (compare any two snapshots).

**Schema-aware diff — Text char-level** — Text now diffs to codepoint runs by char id: `TextInsert{index,text}` (at its index in the new text) / `TextDelete{index,text}` (at its index in the old text), consecutive same-op codepoints coalesced into runs, deletes before inserts (#110). Char-id identity makes it exact (no heuristic alignment); codepoint-indexed, so it counts in codepoints not bytes. Realizes ARCHITECTURE §Schema-Aware-Diff "Text values produce char-level diffs". List element-level detail, XmlElement/marks/attrs diffs (unbuilt primitives), renderers, and the SDK surface remain follow-ons (see Queue).
...
- **Schema-aware diff — deeper + SDK surface** — atop the core primitive-tree diff (#109) + Text char-level (#110): **List element-level diff** (insert/delete by node id, reusing the stable-id addressing the undo layer computes — Text already done), **XmlElement / marks / attrs diffs** (blocked — those primitives aren't built), default **renderers** (ARCHITECTURE §Schema-Aware-Diff "engine ships sensible default renderers; apps can override"), and the **SDK surface** (expose `diff` over FFI/wasm/Python/Go against two state blobs — pairs with `version_state` + export). → *Versioning / Schema-Aware Diff*. (v0.2, partly blocked)
