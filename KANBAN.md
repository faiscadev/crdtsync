...
- **Atomic transactions — SDK surface** — core (#102) and the client/server drive path (#103, `ClientSession::atomic_edit`) landed. Remaining: an `atomic_edit`/`atomic_transact` batch-building entry point over FFI + Python/Go/wasm, returning the tagged ops to broadcast (a closure-shaped batch, or a begin/emit/commit handle for the C ABI which has no closure). Atomic-tx **undo** ("a transaction is naturally an undo intention") and scope constraints (one branch/zone/schema version, member cap) are gated on the branch/zone/schema layers. → *Transactions*. (v0.2)
