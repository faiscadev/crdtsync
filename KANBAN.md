...
**Named versions (wire)** — seven wire messages (tags 11–17): client→server `VersionCreate`/`VersionRename`/`VersionDelete`/`VersionList`/`VersionFetch`; server→client `Versions{names}` (state-based ack + list reply) and `VersionState{name,seq,state}` (fetch reply). Codec + round-trip suite; total decode (#88). Server `step` handling + SDK surface are the next slices (see Next). Restore-as-branch + auto-triggers stay blocked.
...
- **Named versions — server handling + SDK** — the `Hub` index (#86), durable persistence (#87), and wire messages (#88) landed. Next buildable slices: server `step`/registry handling of the version requests (create/rename/delete → `Versions` reply gated on `Write`; list/fetch → `Versions`/`VersionState` gated on `Read`), then the client `ClientSession` version view + issue methods, then the FFI/Python/Go/wasm surface. Restore-as-branch and auto-version triggers stay blocked (branch layer / engine-event hooks). → *Versioning*. (v0.2)
