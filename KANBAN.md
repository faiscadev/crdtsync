...
- **UndoManager — remaining** — scalar undo landed (#94, root Register + Counter). Next slices: nested paths (undo an edit inside a nested Map), then list/text revival (undo of an insert = delete; undo of a delete must re-insert the value — element-revival semantics), then grouping multiple edits into one intention (a transaction = one undo step, per ARCHITECTURE §Undo), and the SDK surface. → *Undo/Redo*. (v0.2)
