...
- **Named versions — wire + SDK** — the `Hub` index (#86) and its durable persistence (#87) landed. Next buildable slice: wire messages (create/list/rename/delete + fetch a version's state) so a client drives versioning, and the SDK surface over them. The wire shape is unpinned by ARCHITECTURE — a request/response sub-protocol to design. Restore-as-branch and auto-version triggers stay blocked (branch layer / engine-event hooks). → *Versioning*. (v0.2)
