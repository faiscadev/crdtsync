...
#[test]
fn a_live_reader_is_revealed_a_lowered_tag_on_a_node_it_already_holds() {
    // The shell's payload is the node's tag read off live state, and a tag is a meet
    // over the claims that named it (C44) — so a claim landing after the shell can
    // lower it. That claim is an `XmlInsertChild` into the node's *birth* list, which
    // this reader may not read: it is served neither the claim nor, on a move gate
    // alone, a second shell. It would hold a tag its own document can never revise,
    // while a fresh joiner reads the meet — two readers of one node, different bytes.
    let mut r = registry();
    let alice = auth(&mut r, 1, "t-alice");
    assert!(subscribe(&mut r, alice));
    r.take_outbox(alice);
    ...
    submit(&mut r, alice, xml_fragment(&mut alice_doc, &col(COL_A)));
    submit(&mut r, alice, xml_fragment(&mut alice_doc, &col(COL_B)));
    let birth = xml_insert_element(&mut alice_doc, &col(COL_B), 0, b"card");
    let insert = birth
        .iter()
        .find(|op| matches!(op.kind, OpKind::XmlInsertChild { .. }))
        .expect("the child insert")
        .clone();
    submit(&mut r, alice, birth);
    r.take_outbox(alice);

    // bob joins, then the card moves into his readable column: he is shelled at `card`.
    let bob = auth(&mut r, 2, "t-bob");
    assert!(subscribe(&mut r, bob));
    let mut live = Document::new(cid(2));
    for op in received_ops(&mut r, bob) {
        live.apply(&op);
    }
    submit(
    ...
    assert_eq!(
        board_render(&live),
        "colA=frag(card()) colB=absent",
        "the live reader holds the card under its first tag",
    );

    // The twin: one stamp, a distinct OpId, a smaller tag. Admissible — dedup is on
    // `OpId` — and it lands in colB, which bob may not read.
    let mut twin = insert.clone();
    twin.id.seq = 9_000;
    if let OpKind::XmlInsertChild { tag, .. } = &mut twin.kind {
        *tag = Some(b"aaa".to_vec());
    }
    assert_eq!(twin.stamp, insert.stamp, "the twin carries one stamp");
    alice_doc.apply(&twin);
    submit(&mut r, alice, vec![twin]);
    // The corrected shell is the whole of what bob is sent: the claim itself is
    // denied him, and the shell's payload and identity both carry the tag.
    let after_twin = received_ops(&mut r, bob);
    assert!(
        after_twin.iter().any(
            |op| matches!(&op.kind, OpKind::XmlReveal { tag, .. } if tag.as_deref() == Some(b"aaa"))
        ),
        "the retag must re-emit the shell: {after_twin:?}",
    );
    for op in &after_twin {
        live.apply(op);
    }

    // A reader joining after the twin reads the meet; the live reader must agree.
    ...
    r.take_outbox(alice);
    let _ = received_ops(&mut r, bob);
    assert!(subscribe(&mut r, carol));
    let mut fresh = Document::new(cid(3));
    for op in received_ops(&mut r, carol) {
        fresh.apply(&op);
    }

    assert_eq!(
        board_render(&fresh),
        "colA=frag(aaa()) colB=absent",
        "a fresh joiner reads the meet of the two claims",
    );
    assert_eq!(
        board_render(&live),
        board_render(&fresh),
        "the live reader kept a tag its own document can never revise",
    );
}
