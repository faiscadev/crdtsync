...
The remaining halves of the constraint line are not enforced at a seam of their own. A document binds one schema version and a channel's replica one branch, so a commit has no second one to span — a schema rebind is a settle-point operation, not something done inside an open transaction. The prohibition on migration ops in a transaction is stated but unenforced.
