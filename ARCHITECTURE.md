...
- The **handshake range-check is the guard**: a recipient that cannot be reached from the document's version across a back-compatible path (i.e. a breaking gap with no inverse) is **refused at handshake with `onUpdateRequired`**, before it is ever a subscriber — so a down-translation at fan-out only ever traverses invertible edges. Forward-only is the sole breakpoint; a back-compatible gap never rejects. What it reaches for is the room's **op-version high-water** — the worst case its merged state embodies, which tracks the state and so survives compaction — read against the room's governing binding, since the number lives in that app's version space. Both are **replicated room metadata**, carried on the same frames the creator rides: a replica serves reads, and a promotion makes one a leader, so a node that held the room's state without them would refuse nobody — and, since a replicated batch is served untranslated, hand the admitted joiner ops its own version cannot model. A joined subscriber a later write or frame opens the high-water past is evicted with the same signal, on the same predicate, wherever the lift arrives.
...
**The shared history is shared, not owned, so it is only as durable as `main`'s retained log.** Compaction folds that log into the replica and lifts the floor, and a whole-replica install (the below-floor follower state-transfer) lands an empty log at a raised floor — either one can pass a live fork point, at which point the records a live-log fork is made of exist in no stream that branch can reach. **A catch-up whose subscriber still needs part of that base refuses rather than serving the retained remainder**: a short delta would catch a fresh subscriber up to the branch's divergent tail over nothing, tell it it is at the head, and let it edit from there. The bound is what the subscriber needs, and it takes both ends of the window — one standing at or above the floor was dropped nothing it still needs, and one at or above its own fork point needs none of the base however far the floor has since run — so the refusal reaches only a subscriber below both. This makes the branch honestly unservable, not whole, and the cost is not an aging one: **a fork taken on a room whose floor has already left zero is unservable from birth**, since the log its shared base names is already gone, and no seam repairs it — deleting and re-forking the name reaches the same missing log. That is a compacted room, and it is also every **cloned or imported** room, which is born with its floor at its head because a whole-replica install lands an empty log at the op count its state carries — so a fork on a clone is refused with no compaction anywhere. (A room still at floor zero is untouched: a fork there shares nothing to lose.) A live-log fork is the wrong primitive on such a room rather than a decaying one. **Retaining a live-log fork's base** — materializing an owned base for every fork the floor is about to pass, and for every fork taken above a floor that has already risen, which trades the "only divergent ops" storage claim above for one replica copy per fork — is the open half (C88), and where a fork off `main` sits at a head that counts `main`'s whole history, that copy is the live replica rather than a reconstruction — a shortcut with conditions, since a head lowered by a whole-replica install no longer counts it, and a fork off a *branch* clamps to that branch's head instead. A branch that owns its base (a restore, a publish, a snapshot fork) is unaffected either way, and so is `main`, whose replica *is* what the log folded into.
...
**A redaction withholds a reader's own authorship from it, and every seam that does has to hand back the ids.** The filters are authorship-blind by construction — a path verdict and a partition verdict are properties of the op, not of who wrote it — so every narrowed serve withholds some of its own recipient's run. That is not merely missing content: a mint reads two records, the per-client sequence it has not published and its own entry in the id-space high-water, and a withheld op holes both. The first loses the next write to every peer's dedup set; the second re-derives every id taken from a stamp alone (an ACL tuple's, a ranged element's, an XML sequence child's), so the element it names is swallowed at ingest. Both are silent. **A snapshot carries them in its state** — the projection scrubs the causal frontier and the id-space record of every *other* author and keeps the recipient's own, which the recipient originated. **A delta carries them in a frame ahead of it** — `Message::Frontier` names the sequences the delta withholds and the position their ops reach, as sequences under one identity rather than ids, so the frame can only reach the id space its own recipient authors in; it leads the delta, because a provider opens the socket to app traffic on a frame and the other order leaves a window in which an edit authors against a replica still holding the hole. Naming a sequence is not refusing its op: the recipient reserves it for the mint alone and still folds the op if a widened grant or a resumed subscription delivers it. A *live* fan-out needs neither carrier, on the invariant that a replica applies its own op at authoring time — a property of one replica per identity, not of the frame. What a recipient learns is bounded by who may be served its channel: the identity is declared, and authenticating the declaration is the transport's job (§Client ID).
