...
A group id must be unique for the life of the *client*, not of one replica object — a receiver buckets buffered members by author and group id, so an id counter restarted by a snapshot restore would merge a stale partial group with a fresh one and both commit a mixed set and strand the remainder. Deriving the id from an op seq, which is already unique per client and already carried across a restore, is what makes that structurally impossible.
