...
Core is implemented in **C**, exported as WASM and as a stable C ABI, wrapped by thin SDKs per language.
...
- portability everywhere a CRDT must run (browser via WASM, every native language via stable ABI, embedded contexts, edge runtimes)
- zero-runtime, predictable performance, no GC pauses
- universally embeddable — every language has FFI to C, no exceptions
- single canonical ABI is the lingua franca of cross-language interop
- mature toolchain (compilers, debuggers, fuzzers, sanitizers, static analyzers)
- WASM compiles cleanly from C
- minimal binary size matters for browser delivery

CRDTs and distributed systems still benefit heavily from explicit modeling, careful invariant handling, correctness-oriented architecture — discipline imposed by code review, fuzzing, and a property-based test suite rather than by the type system.
