...
**Admission is one-directional.** A dialing node proves itself to the node it dials; the node it dials proves nothing back, so a `ReplicaAck` is taken on trust and anything holding a member's advertise address can make a leader believe a write is durably replicated. Making the peer handshake mutual belongs with the encrypted peer transport, where there is a second identity to prove.
