...
A group id must outlive the replica object that minted it — a receiver buckets buffered members by author and group id, so an id counter restarted by a snapshot restore merges a stale partial group with a fresh one, commits a mixed set, and strands the remainder. The id is therefore an op seq: minted by the counter that already distinguishes the group's own members, and carried rather than reset by the restore paths that preserve identity, so a group id is exactly as unique as the op ids beside it and needs no separate persistence.
...
**A channel's replica is its own author.** Two channels of one connection can name the same room — a whole-room subscription beside a zone-scoped one, two zones, a branch beside the default — and each holds an independent replica minting op seqs and lamports from its own zero. So channel `c` authors under `client_id.for_channel(c)`: the declared id itself on the first channel (an app that persists a client id keeps it on its primary subscription), a UUIDv5 over `(declared id, c)` on every further one. The derivation is pure, so the session driver re-derives it from the Hello id and the channel an op batch names, and refuses a batch carrying any other identity. Without it both replicas mint identical `OpId`s and identical stamps for unrelated edits, and every consumer keyed on them — a peer's dedup set, the room's log, the `(client, TxId)` transaction bucket, a counter's per-client tally — folds one channel's writes into the other's and drops them.

Five docs in five tabs = five connections (per-tab `client_id`). Five docs in one tab = one connection with five channels, five replica identities derived from that tab's.
