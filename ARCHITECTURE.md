...
- **The leaf/container boundary is explicit, not inferred by shape.** A **leaf** is written with `map.set(key, scalar)` / `list.insert(i, scalar)` and holds one `Scalar`. A **container** is created with the explicit `getMap` / `getList` / `getText` / `getXml` accessor. The SDK does **not** deep-seed a nested container from a plain object/array passed to `set` in v1 — `set(k, {a:1})` is a type error, not an implicit `getMap(k)` + recursion. (Deep-seed / plain-object-to-container conversion is an enumerated ergonomic follow-on, KANBAN; keeping v1 explicit avoids ambiguity over whether a JS value is a leaf or a subtree and keeps marshaling total.) This is the load-bearing marshaling rule other languages must match: **native scalar ⇒ leaf; explicit accessor ⇒ container.**
