...
let test_equal_scalar_reflexive () =
  ...
  in
  List.iter (fun s -> Alcotest.(check bool) "scalar = itself" true (Value.equal_scalar s s)) cases
...
let test_equal_scalar_distinguishes_variants () =
  ...
  Alcotest.(check bool) "Int 1L <> Bool true" false (Value.equal_scalar (Int 1L) (Bool true));
  Alcotest.(check bool) "Int 0L <> Bool false" false (Value.equal_scalar (Int 0L) (Bool false));
  Alcotest.(check bool) "Float 0.0 <> Int 0L" false (Value.equal_scalar (Float 0.0) (Int 0L));
  Alcotest.(check bool) "Null <> Bool false" false (Value.equal_scalar Null (Bool false))

let test_equal_scalar_string () =
  Alcotest.(check bool) "same string" true (Value.equal_scalar (String "abc") (String "abc"));
  Alcotest.(check bool) "different string" false (Value.equal_scalar (String "abc") (String "abd"));
  Alcotest.(check bool) "empty vs nonempty" false (Value.equal_scalar (String "") (String "x"))
...
let test_equal_scalar_int () =
  ...
  Alcotest.(check bool) "max_int" true (Value.equal_scalar (Int Int64.max_int) (Int Int64.max_int))

let test_equal_scalar_float () =
  Alcotest.(check bool) "same float" true (Value.equal_scalar (Float 1.5) (Float 1.5));
  Alcotest.(check bool) "different float" false (Value.equal_scalar (Float 1.5) (Float 1.6));
...
let test_equal_value_element_inner_changes () =
  ...
  Alcotest.(check bool) "Element a <> Element b" false (Value.equal (Element a) (Element b))
...
let () =
  Alcotest.run "value"
    [
      ( "equal_scalar",
        [
          Alcotest.test_case "reflexive on all variants" `Quick test_equal_scalar_reflexive;
          ...
          Alcotest.test_case "float equality (incl NaN = NaN)" `Quick test_equal_scalar_float;
        ] );
      ( "pp_scalar",
        [ Alcotest.test_case "distinct outputs across variants" `Quick test_pp_scalar_distinct ] );
      ( "equal",
        [
          ...
          Alcotest.test_case "Scalar inner change" `Quick test_equal_value_scalar_inner_changes;
          Alcotest.test_case "Element inner change" `Quick test_equal_value_element_inner_changes;
        ] );
      ("pp", [ Alcotest.test_case "output nonempty for all variants" `Quick test_pp_value_nonempty ]);
    ]
