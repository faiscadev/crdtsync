(** Tests for [Crdtsync_crdt.Envelope] and [Crdtsync_crdt.Op]. Black-box: only the .mli
...
let fresh_envelope ?(tx = None) () =
  Envelope.make ~op_id:(fresh_op_id ()) ~actor_id:"user_42" ~room:"doc-1" ~branch:"main"
    ...
    ~wall_time:(Wall_time.of_ms 1733000000000L)
    ~op:Op.Placeholder ?tx ()
...
let test_make_round_trips_all_fields () =
  ...
  let env =
    Envelope.make ~op_id ~actor_id:"alice" ~room:"room-1" ~branch:"feature/x"
      ~zone:"zone-a" ~schema_version:3 ~lamport ~wall_time ~op:Op.Placeholder ()
  in
  ...
  Alcotest.(check int64)
    "wall_time round-trips" 1700000000000L
    (Wall_time.to_ms env.wall_time);
  Alcotest.(check bool) "op round-trips" true (Op.equal_kind env.op Op.Placeholder);
  Alcotest.(check bool)
    "tx defaults to None when ?tx omitted" true (Option.is_none env.tx)
...
let test_equal_same_fields_distinct_values () =
  ...
  let mk () =
    Envelope.make ~op_id ~actor_id:"u" ~room:"r" ~branch:"b" ~zone:"z" ~schema_version:1
      ~lamport ~wall_time:(Wall_time.of_ms 0L) ~op:Op.Placeholder ()
  in
  Alcotest.(check bool)
...
let test_equal_differing_tx () =
  ...
  let mk tx =
    Envelope.make ~op_id ~actor_id:"u" ~room:"r" ~branch:"b" ~zone:"z" ~schema_version:1
      ~lamport ~wall_time:(Wall_time.of_ms 0L) ~op:Op.Placeholder ?tx ()
  in
  let t1 = Uuid_v7.v () in
  let t2 = Uuid_v7.v () in
  let none = mk None in
  ...
  Alcotest.(check bool)
    "tx differing id => not equal" false
    (Envelope.equal mem mem_other_id)
...
let test_pp_tx_role_nonempty () =
  ...
  Alcotest.(check bool)
    "Member and Commit print differently" true
    (not (String.equal m c))
    ...
    [
      ...
      ( "equal",
        [
          ...
          Alcotest.test_case "same fields => equal" `Quick
            test_equal_same_fields_distinct_values;
          Alcotest.test_case "differing op_id => not equal" `Quick
            test_equal_differing_op_id;
      ...
      ( "tx_role",
        [
          ...
          Alcotest.test_case "pp_tx_role nonempty and distinct" `Quick
            test_pp_tx_role_nonempty;
        ] );
      ( "pp",
        [ Alcotest.test_case "envelope pp nonempty" `Quick test_pp_envelope_nonempty ] );
    ]
